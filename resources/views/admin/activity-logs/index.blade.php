    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ __('Audit log records of user actions and system changes.') }}</p>
            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-slate-50/80 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Subject Entity') }}</th>
                                        ...
                                        <tr class="hover:bg-slate-50/70 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                @if($activity->subject_type)
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 shadow-2xs">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                <span class="font-semibold text-gray-900 block">{{ $activity->created_at->format('Y-m-d H:i:s') }}</span>
                                                <span class="text-xs text-gray-400 mt-0.5">{{ $activity->created_at->diffForHumans() }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
