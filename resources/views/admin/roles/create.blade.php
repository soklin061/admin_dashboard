                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.roles.store') }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required autofocus placeholder="e.g. Manager" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            @error('name')
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5">
                                        @foreach($groupPerms as $permission)
                                            <label class="inline-flex items-center p-2 rounded-xl bg-white border border-gray-100 hover:border-indigo-200 transition cursor-pointer shadow-2xs">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="perm-checkbox rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 h-4 w-4">
